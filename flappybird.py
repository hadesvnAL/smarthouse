...
bg=pygame.image.load('sr/bg.png')
bw=bg.get_width()
...
class TBlock(pygame.sprite.Sprite):
   def __init__(self,x,h1):
      super().__init__()
      self.image=pygame.image.load('sr/tp.png')
...
class BBlock(pygame.sprite.Sprite):
   def __init__(self,x,h2):
      super().__init__()
      self.image=pygame.image.load('sr/bp.png')
...
class Game:
   ...
   def msg(self,text,x,y,color,size):
      self.font=pygame.font.SysFont('Arial',size,bold=1)
   ...
   def over(self):
      ...
      while wait:
         ...
         self.msg("Press Enter to Play Again",dw-445,dh+200,red,40)
         pygame.display.flip()
      self.new()
   def scores(self):
         self.msg("Score:"+str(self.score),dw-40,150,black,30)
